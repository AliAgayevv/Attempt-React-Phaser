...
export default class SnakeScene extends Phaser.Scene {
  ...
  private apple;
  private goldenApple: Phaser.GameObjects.Arc;
  ...
  create() {
    ...
    for (let i = 0; i < 3; i++) {
      const segment = this.add.rectangle(
        this.snakeStartX,
        this.snakeStartY + i * this.cellSize,
        this.cellSize,
        this.cellSize,
        0x00ff00
      );
  ...
  moveSnake() {
    ...
    const newHead = this.add.rectangle(
      (head.x + this.direction.x * this.cellSize + window.innerWidth) %
        window.innerWidth,
      (head.y + this.direction.y * this.cellSize + window.innerHeight) %
        window.innerHeight,
      this.cellSize,
      this.cellSize,
      0x00ff00
  ...
  checkCollision(
    ...
    const obj1Transform =
      obj1 as unknown as Phaser.GameObjects.Components.Transform;
    const obj2Transform =
      obj2 as unknown as Phaser.GameObjects.Components.Transform;
    return (
      obj1Transform.x === obj2Transform.x && obj1Transform.y === obj2Transform.y
    );
  }
